...
function createFinalProgressForm() {
  ...
  var form = FormApp.create("End of Year Evaluation");
  ...
  form.addTextItem().setTitle("Grade");
...
function preFilledURLsFINAL(sheet, theData, formID){
  ...
  for(var i = 0; i < allData.length; i++){
    ...
    resp.withItemResponse(items[0].asListItem().createResponse(allData[i].studentName));
    resp.withItemResponse(items[1].asTextItem().createResponse(allData[i].studentId));
    resp.withItemResponse(items[2].asTextItem().createResponse(allData[i].grade));
    resp.withItemResponse(items[3].asTextItem().createResponse(allData[i].teacherName));
  ...
  sheet.getRange(2, lcol + 1).setFormula("=arrayformula(HYPERLINK(H2:H,A2:A))"); //NEED a way to offset this!!
